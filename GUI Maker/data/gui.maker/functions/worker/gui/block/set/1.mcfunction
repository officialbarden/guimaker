...
$execute as @e[type=marker,tag=gm.gui.block.temp,sort=nearest,limit=1] run function gui.maker:worker/gui/item_modify/1 {GUI_ID:$(GUI_ID),PAGE:$(page_number)}
$execute as @e[type=marker,tag=gm.gui.block.temp,sort=nearest,limit=1] run function gui.maker:worker/gui/change_name/1 {GUI_ID:$(GUI_ID),PAGE:$(page_number)}
