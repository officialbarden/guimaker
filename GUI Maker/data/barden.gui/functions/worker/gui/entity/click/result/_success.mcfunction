...
$execute as @e[type=chest_minecart,sort=nearest,limit=1,tag=!bgui.page.changed] run function barden.gui:worker/gui/entity/click/result/extra_functionalities/bgui_changepage with storage barden:gui.page.init PAGEINIT[{uuid:$(minecartuuid)}].page[0].tag.bguichangetoPageData
