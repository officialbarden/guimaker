...
$tellraw @s {"text":"[CHANGE VALUES]","color":"green","clickEvent":{"action":"suggest_command","value":"/function barden.gui:maker/commands/editpage/page/changevalues {FUNCTION_FILE:\"\",ITEM_MODIFIER:\"\",CHANGE_TO_GUI_ID:\"\",CHANGE_TO_PAGE_INDEX:\"\",GUI_ID:$(GUI_ID),PAGE:$(PAGE)}"},"hoverEvent":{"action":"show_text","contents":"Replace Values with appropriate values"}}
tellraw @s {"text": "\n"}
